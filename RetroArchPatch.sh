...
rm config.def.h.mod
count=`grep "config_set_defaults(global_get_ptr())" retroarch.c | wc -l`; if [[ $count -eq 0 ]]; then echo "Errore global_get_ptr()"; fi
...
count=`grep "case CMD_EVENT_UNLOAD_CORE:" retroarch.c | wc -l`; if [[ $count -eq 0 ]]; then echo "CMD_EVENT_UNLOAD_CORE"; fi
